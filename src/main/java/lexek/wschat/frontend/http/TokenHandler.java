    ...
    protected void handle(Request request, Response response) throws Exception {
        ...
        if (user != null) {
            if (request.method() == HttpMethod.GET) {
                UserAuthDto auth = authenticationManager.getAuthDataForUser(user.getId(), "token");
                if (auth != null) {
                    response.jsonContent(ImmutableMap.of("token", auth.getAuthenticationKey()));
                } else {
                    response.jsonContent(ImmutableMap.of("token", ""));
                }
