...
CREATE TABLE `chatter` (
	...
	`role` TINYTEXT NOT NULL COLLATE 'utf8mb4_unicode_ci',
	`banned` BIT(1) NOT NULL,
	`timeout` BIGINT(20) NULL DEFAULT NULL,
	PRIMARY KEY (`id`),
	UNIQUE INDEX `UNIQUE_ROOM_PLUS_USER` (`room_id`, `user_id`),
	INDEX `FK_USER` (`user_id`),
	CONSTRAINT `FK_CHATTER_USER` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON UPDATE CASCADE ON DELETE CASCADE,
	CONSTRAINT `FK_CHATTER_ROOM` FOREIGN KEY (`room_id`) REFERENCES `room` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `announcement` (
	...
	CONSTRAINT `FK_ANNOUNCEMENT_ROOM` FOREIGN KEY (`room_id`) REFERENCES `room` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `history` (
	...
	INDEX `FK_USER` (`user_id`),
	CONSTRAINT `FK_HISTORY_ROOM` FOREIGN KEY (`room_id`) REFERENCES `room` (`id`) ON UPDATE CASCADE ON DELETE CASCADE,
	CONSTRAINT `FK_HISTORY_USER` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `journal` (
	...
	`action_description` TEXT NULL COLLATE 'utf8mb4_unicode_ci',
...
CREATE TABLE `poll` (
	`id` BIGINT(20) NOT NULL AUTO_INCREMENT,
	`room_id` BIGINT(20) NOT NULL,
	`question` TEXT NOT NULL COLLATE 'utf8mb4_unicode_ci',
	`open` BIT(1) NOT NULL,
	PRIMARY KEY (`id`),
	INDEX `FK_POLL_ROOM` (`room_id`),
	CONSTRAINT `FK_POLL_ROOM` FOREIGN KEY (`room_id`) REFERENCES `room` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `poll_option` (
	...
	CONSTRAINT `FK_POLL_OPTION_POLL` FOREIGN KEY (`poll_id`) REFERENCES `poll` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `poll_answer` (
	...
	CONSTRAINT `FK_POLL_ANSWER_OPTION` FOREIGN KEY (`poll_id`, `selected_option`) REFERENCES `poll_option` (`poll_id`, `option`) ON UPDATE CASCADE ON DELETE CASCADE,
	CONSTRAINT `FK_POLL_ANSWER_POLL` FOREIGN KEY (`poll_id`) REFERENCES `poll` (`id`) ON UPDATE CASCADE ON DELETE CASCADE,
	CONSTRAINT `FK_POLL_ANSWER_USER` FOREIGN KEY (`user_id`) REFERENCES `user` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `session` (
	...
	CONSTRAINT `FK_SESSION_USERAUTH` FOREIGN KEY (`userauth_id`) REFERENCES `userauth` (`id`) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE `ticket` (
	...
	`category` TEXT NOT NULL COLLATE 'utf8mb4_unicode_ci',
	`text` TEXT NOT NULL COLLATE 'utf8mb4_unicode_ci',
	`admin_reply` TEXT NULL COLLATE 'utf8mb4_unicode_ci',
